...
FROM Melons JOIN Order_items ON (melon_id=melons.id) 
WHERE order_id = 2725;
...
SELECT salespeople.first_name, salespeople.last_name, SUM(order_total), SUM(order_total) * 0.15 AS commission
FROM orders LEFT JOIN salespeople
ON (orders.salesperson_id = salespeople.id)
WHERE salespeople.id IS NOT NULL
